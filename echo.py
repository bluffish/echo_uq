...
def get_label_mean_and_std(dataset: torch.utils.data.Dataset,
                           samples: int = 128,
                           batch_size: int = 8,
                           num_workers: int = 4):
    """
    Computes mean and std of labels from a PyTorch dataset.

    Args:
        dataset (torch.utils.data.Dataset): Dataset where each item is a tuple (x, y).
        samples (int or None): Number of random samples to compute stats over. If None, use entire dataset.
        batch_size (int): Batch size for loading.
        num_workers (int): Number of workers for DataLoader.

    Returns:
        (mean, std): Tuple of float or np.ndarray depending on label shape.
    """
    if samples is not None and len(dataset) > samples:
        indices = np.random.choice(len(dataset), samples, replace=False)
        dataset = torch.utils.data.Subset(dataset, indices)
    ...
    for (_, y) in tqdm.tqdm(dataloader):
        y = y.view(y.size(0), -1)  # flatten each label if needed
        n += y.size(0)
        s1 += torch.sum(y, dim=0).numpy()
        s2 += torch.sum(y ** 2, dim=0).numpy()
